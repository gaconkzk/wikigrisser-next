...
import type { ClassWorkbookRow, MaxStatsWorkbookRow } from '../types/spreedsheet';
...
export class DBSingleton {
	...
	getMaxStats(): MaxStatsWorkbookRow[] {
		...
		let notDone = true;
		let rowCounter = 2;
		const maxStatsArr: MaxStatsWorkbookRow[] = [];
	...
	private getHeroData(name: string): Hero {
		const rowNumber = this.findHeroRow(name);
	...
	private getStartingClass(rowNumber: number): Class {
		const soldiers = this.getWorkbookHeroRowValue(rowNumber, 'BV')?.split(',') || [];
		return {
	...
	private getTopLevelClassPath(rowNumber: number, startingCol: string): Class[] {
		const skill1 = this.getNextKey(startingCol);
		const children = this.getSecondLevelClassPath(rowNumber, this.getNextKey(skill1));
		const name = this.getWorkbookHeroRowValue(rowNumber, startingCol);
		let outerClass: Class[] = [
			{
				name,
				skills: [this.skillsMap[this.getWorkbookHeroRowValue(rowNumber, skill1)] || null],
	...
	private getSecondLevelClassPath(rowNumber: number, startingCol: string): Class[] {
		...
		const maxStats =
			this.maxStats.find((stats) => stats.class === name && stats.name === heroName)?.stats || null;
		...
		let classes: Class[] = [
			{
				name,
				skills: [
					this.skillsMap[this.getWorkbookHeroRowValue(rowNumber, skill1Pos)] || null,
					this.skillsMap[this.getWorkbookHeroRowValue(rowNumber, skill2Pos)] || null
				],
	...
	private getSkillsPerHero(hero: Hero): HeroSkillsTouple {
		return [
			hero.prettyName,
			[...(hero.SpClass?.skills || []), ...this.getSkillsPerClass(hero.startingClass)]
		];
	}
	...
	private reduceHeroSkillsTouplesToSkillMap(
		...
		skills.forEach((skill) => {
			if (skill) {
				accumulator[skill.name] = [...(accumulator[skill.name] || []), heroName];
			}
		});
		return accumulator;
	}
	...
	private getNextKey(key: string): string {
		...
		if (key === 'Z' || key === 'z') {
			return (
				String.fromCharCode(key.charCodeAt(0) - 25) + String.fromCharCode(key.charCodeAt(0) - 25)
			); // AA or aa
		} else {
